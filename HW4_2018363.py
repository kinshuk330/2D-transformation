...
plt.ion()#to make plot interactive
c=0#required to label conversions
...
def rotate(x,Q,shape='polygon'):#for rotating the matrix of co-ordinates
	...
	'''
	Q=math.radians(Q)#conversion of degrees into radians
...
def show_details(x,shape='polygon'):#for showing various details after every transformation on terminal
	if shape=='polygon':
...
def query_handling(newx,x,shape):#for handling queries
	while 1:
